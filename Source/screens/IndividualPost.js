...
import
{ PostCard } from '../shareable/customCard'
import
{ MaterialCommunityIcons, Entypo } from '@expo/vector-icons';
import
{ Input } from "react-native-elements";
import
{ FontAwesome, AntDesign } from '@expo/vector-icons';
...
const IndividualPostScreen = ( props ) => {
    ...
    const loadComments = async () => {
        ...
        setLoading( true )
        firebase.firestore().collection( "posts" ).doc( posts.id ).collection( 'comment_writer' ).orderBy( "written_at", "desc" ).onSnapshot( ( querySnapshot ) => {

            let temp_comments = [];
            querySnapshot.forEach( ( doc ) => {

                temp_comments.push( { id: doc.id, data: doc.data() } );
        ...
        <AntDesign name="checksquare"
            ...
            onPress={
                function ()
                {
                    ...
                    firebase.firestore().collection( "notifications" ).doc( posts.data.userId ).collection( "notification_details" ).add( { post: posts, name: currUser.displayName, body: "commented on your post" } )
                    for ( let omment of comments )
                    {

                        console.log( omment )
                        firebase.firestore().collection( "notifications" ).doc( omment.data.writer_id ).collection( "notification_details" ).add( { post: posts, name: currUser.displayName, body: "replied your comment" } )
        ...
        <FlatList data={comments}
            ...
            renderItem={
                function ( { item } )
                {
                    ...
                    return (<CommentList comments={item}/>)
