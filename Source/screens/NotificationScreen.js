...
import
{ View, ActivityIndicator, FlatList, TouchableOpacity } from 'react-native'
...
const NotificationScreenActivity = ( props ) => {
    ...
    const loadNotification = async () => {
        ...
        setLoading( true )
        firebase.firestore().collection( "notifications" ).doc( uid ).collection( "notification_details" ).onSnapshot( ( querySnapshot ) => {
    ...
    return (<View style={
        ...
        {
        !loading ? <FlatList data={notifications}
                ...
                function ( { item } )
                {
                    ...
                    return (<TouchableOpacity onPress={
                        function ()
                        {
                            ...
                            props.navigation.navigate( "IndivialPost",
                            { posts, currUser, date } );

                        }
                    }>
                        <NotificationList notificatiions={item}
                            nav={props}/>
                    </TouchableOpacity>)
                }
            }/> : <View style={
