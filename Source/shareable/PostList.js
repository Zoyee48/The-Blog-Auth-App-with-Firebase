        ...
        <AntDesign name={iconName}
            ...
            onPress={
                function ()
                {
                    ...
                    firebase.firestore().collection( "posts" ).doc( posts.id ).collection( "likers" ).doc( currUser.uid ).set( { liker: currUser.displayName } )
                    firebase.firestore().collection( "posts" ).doc( posts.id ).update( {

                        likes: likeCount + 1

                    } )
                    firebase.firestore().collection( "notifications" ).doc( posts.data.userId ).collection( "notification_details" ).add( { post: posts, name: currUser.displayName, body: "Liked your post" } )
                    let a = likeCount + 1
